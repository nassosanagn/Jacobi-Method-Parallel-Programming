 ...
 **********************************************************/
long double checkSolution(long double xStart, long double yStart,
                          int maxXCount, int maxYCount,
                          long double *u,
                          long double deltaX, long double deltaY,
                          double alpha) {
#define U(XX, YY) u[(YY)*maxXCount + (XX)]
  int x, y;
  long double fX, fY;
  long double localError, error = 0.0;
...
int main(int argc, char **argv) {
  ...
  // Solve in [-1, 1] x [-1, 1]
  long double xLeft = -1.0, xRight = 1.0;
  long double yBottom = -1.0, yUp = 1.0;
  ...
  long double deltaX = (xRight - xLeft) / (n - 1);
  long double deltaY = (yUp - yBottom) / (m - 1);
  ...
  u = (long double *)calloc(((size_n + 2) * (size_m + 2)), sizeof(long double)); //reverse order
  u_old = (long double *)calloc(((size_n + 2) * (size_m + 2)), sizeof(long double));
...
#define DST(XX, YY) u[(YY) * (size_n + 2) + (XX)]
    ...
    long double fX, fY;
    long double updateVal;
    long double f;

    // Coefficients
    long double cx = 1.0 / (deltaX * deltaX);
    long double cy = 1.0 / (deltaY * deltaY);
    long double cc = -2.0 * cx - 2.0 * cy - alpha;
    error = 0.0;

    //Define Column and Row Types
    MPI_Datatype column_type;
    MPI_Type_vector(size_n, 1, size_n + 2, MPI_LONG_DOUBLE, &column_type);
    MPI_Type_commit(&column_type);

    MPI_Datatype row_type;
    MPI_Type_contiguous(size_n, MPI_LONG_DOUBLE, &row_type);
    MPI_Type_commit(&row_type);

    MPI_Request sendRequests[4];
    MPI_Request receiveRequests[4];

    long double receivedRight[size_n]; //create temp right matrix
    long double receivedLeft[size_n];  //create temp left matrix
    ...
    MPI_Irecv(&receivedLeft, size_n, MPI_LONG_DOUBLE, neighbour_ranks[0], 0, comm, &receiveRequests[0]);
    MPI_Irecv(&receivedRight, size_n, MPI_LONG_DOUBLE, neighbour_ranks[1], 0, comm, &receiveRequests[1]);
    MPI_Irecv(&SRC(1, 0), size_n, MPI_LONG_DOUBLE, neighbour_ranks[2], 0, comm, &receiveRequests[2]);
    MPI_Irecv(&SRC(1, size_n + 1), size_n, MPI_LONG_DOUBLE, neighbour_ranks[3], 0, comm, &receiveRequests[3]);
    ...
    MPI_Allreduce(&error, &global_sum, 1, MPI_LONG_DOUBLE, MPI_SUM, MPI_COMM_WORLD); //reduce local error to global sum
  ...
  long double absoluteError = checkSolution(xLeft, yBottom,
                                            size_n + 2, size_m + 2,
                                            u_old,
                                            deltaX, deltaY,
                                            alpha);

  MPI_Allreduce(&absoluteError, &global_error, 1, MPI_LONG_DOUBLE, MPI_SUM, MPI_COMM_WORLD);

  global_error = sqrt(global_error) / (n * m);
  printf("The error of the iterative solution is %Lg\n", global_error);
